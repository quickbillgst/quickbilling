...
export async function GET(
  ...
  try {
    ...
    const html = generateRelievingLetterHTML(letter, tenant);

    return new NextResponse(html, {
      headers: { 'Content-Type': 'text/html' },
    });
  } catch (error) {
    console.error('Error generating relieving letter PDF:', error);
    return NextResponse.json({ error: 'Failed to generate relieving letter' }, { status: 500 });
  }
}

function generateRelievingLetterHTML(letter: any, tenant: any): string {
...
<body>
  ...
  <div class="page">
    <div class="watermark">RELIEVING</div>
    ...
    <div class="content">
      <div class="letter-meta">
        ...
        <div>
          <strong>Date:</strong> ${formatDate(letter.relievingDate)}
        </div>
      </div>
      ...
      <div class="salutation">
        Dear <span class="highlight">${letter.employeeName}</span>,
      </div>
      ...
      <p class="body-text">
        We hereby confirm that ${letter.employeeName} has been relieved from the services of ${companyName}
        effective <span class="highlight">${formatDate(letter.lastWorkingDate)}</span>. All dues have been settled, and there are
        no outstanding obligations from either party.
      </p>
      ...
      ${letter.remarks ? '<p class="body-text"><strong>Note:</strong> ' + letter.remarks + '</p>' : ''}

      <p class="wishes">
        We appreciate the contributions made by ${letter.employeeName} during the tenure and wish all the best
        in future endeavours.
      </p>

      <p class="body-text">
        This letter is being issued upon request for the purpose of records and future employment.
      </p>
